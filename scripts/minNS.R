...
a <- 
  ...
  geom_point(data = subset(ns, ! is.na(labP)), color = 'red', size = 1.5) +
...
b <- 
  ...
  geom_point(data = subset(ns, ! is.na(labC)), color = 'red', size = 1.5) +
...
ggsave(filename = output, plot = ggarrange(a, b, ncol = 2, nrow = 1), width = 170, height = 75, units = "mm")
