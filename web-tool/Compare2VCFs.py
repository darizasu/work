...
parser.add_argument("-q", dest='minQuality', help="Integer. Minimum genotype quality for a genotype call to be taken into account in the comparison. Default is 60", type=int, default=60, required=False)
arguments = parser.parse_args()

# inputCmpVCF = arguments.VCF1
# outputFile = arguments.VCF2

def getLineInfo(line,minQ):
...
def getCommonSNPs(inputVCF1, inputVCF2, minQ):
	...
	with gzip.open(inputVCF1,'r') as inFile1, gzip.open(inputVCF2, 'r') as inFile2:
		...
		for line1 in inFile1:
			...
			if theLine1[0][0] == '#':
				...
				theLine1 = getLineInfo(theLine1, minQ)
		...
		for line2 in inFile2:
			...
			if theLine2[0][0] == '#':
				...
				theLine2 = getLineInfo(theLine2, minQ)
...
try:
	getCommonSNPs(arguments.VCF1, arguments.VCF2, arguments.minQuality)
except IOError:
	try:
		sys.stdout.close()
	except IOError:
